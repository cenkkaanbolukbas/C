...
#include<stdlib.h>
#include<math.h>
int calculate(int section[][10],int size,int averages[],int sum[],int *highestaverage,int *winnersection){
	
	int i,k;
	*highestaverage=0;
	*winnersection=0;
	for(i=0;i<4;i++){  // Having the sum of each section.
		for(k=0;k<10;k++){  
			sum[i]+=section[i][k];
		}
	}
	for(i=0;i<4;i++){  // Finding averages.
		averages[i]=sum[i]/10;
	}
	for(i=0;i<4;i++){  // Finding highest average.
...
int main(){   
	...
	int z=0;
	int y=0;
	int section[4][10],averages[4],sum[4];
	
	for(i=0;i<4;i++){   // Input grades for each section.
		y++;
		printf("Enter the grades of section %d: ",y);
		for(k=0;k<10;k++){
		scanf("%d",&section[i][k]);	
		}
	}
		
	calculate(section,4,averages,sum,&highestaverage,&winnersection);  // Call the function
